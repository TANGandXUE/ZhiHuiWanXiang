...
export class UploadController {
    ...
    async uploadFiles(@Body() body, @UploadedFiles() files, @Req() req) {
        ...
        //写入文件，返回异步文件信息
        let fileInfo = await this.uploadService.writeFiles(files);

        console.log('2');

        //写入数据库
        this.sqlService.uploadFiles(fileInfo)

        //将fileInfo单独取出，并转化成linux路径格式
        let filePathList: string[] = this.datatransService.convertWindowsSlashes(fileInfo.map(item => item.filePath));
