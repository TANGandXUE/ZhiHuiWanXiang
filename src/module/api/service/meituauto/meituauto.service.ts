...
//   private msgId: string; //暂时存储msgId
  ...
  private all_params = {
    ...
    async meitu_auto_startProcessSingle(fileInfo: { fileName: string, fileURL: string }): Promise<string> {
        ...
        console.log('startProcess方法内的msgId: ', response.data.data.msg_id);
  ...
  //调用前两个方法，执行美图智能修图
  async meitu_auto(fileInfos_url_input: Array<{ fileName: string, fileURL: string}>, callback: (fileInfos_url_output) => void) {
    const msgIds: string[] = [];
